...
#include <afxext.h>
#include "..\VTest\VTestDoc.h"
#include "..\VTest\VTestView.h"
...
public:
	virtual HRESULT InitializeDocumentPreview(HWND hWndParent, RECT* prc)
	{
		...
		CCreateContext ctx;
		ctx.m_pNewViewClass = RUNTIME_CLASS(CVTestView);
		...
		m_pDocument->AddRef();
		ctx.m_pCurrentDoc = DYNAMIC_DOWNCAST(CVTestDoc, (CObject*) m_pDocument->GetContainer());
